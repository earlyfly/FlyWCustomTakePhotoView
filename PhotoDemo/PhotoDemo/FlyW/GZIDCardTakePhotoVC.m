...
@interface GZIDCardTakePhotoVC ()<AVCaptureMetadataOutputObjectsDelegate,AVCapturePhotoCaptureDelegate,UIGestureRecognizerDelegate>
...
- (void)setupView {
    ...
    [self.tishiLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.kuangImgView);
        make.centerX.equalTo(self.kuangImgView.mas_left).offset(-20);
    }];
...
- (void)selectImage {
    ...
    // 缩放图片
    UIImage *image = [UIImage image:self.image scaleToSize:CGSizeMake(targetW, targetH)];
...
- (UIImageView *)kuangImgView {
    if (_kuangImgView == nil) {
        _kuangImgView = [[UIImageView alloc] initWithImage:self.areaImage];
    }
...
- (UILabel *)tishiLabel {
    ...
    if (_tishiLabel == nil) {
        ...
        _tishiLabel.text = self.idCardType == GZIDCardTakeTypeBack ? @"请拍摄身份证国徽面，并尝试对其边缘" : @"请拍摄身份证人像面，并尝试对其边缘";
